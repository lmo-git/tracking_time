...
# ============================================
    ...
    st.title("📋 ระบบบันทึกข้อมูลสำหรับพนักงาน")
    ...
    df_scan.columns = df_scan.columns.str.strip()  # Clean headers

    if "ทะเบียนรถ" not in df_scan.columns:
        st.error("❌ ไม่พบคอลัมน์ 'ทะเบียนรถ' ใน Google Sheet 'scan'")
        st.stop()

    if df_scan.empty:
        st.warning("⚠️ ยังไม่มีข้อมูลใน sheet scan")
        st.stop()
    ...
    if plate:
        ...
        if not s3_records.empty:
            ...
            latest_s3_time = latest_row.get("Time3", "")
